...
// ---------- Индексация 16x16 (зигзаг, нормальная ориентация) ----------
int xy2index16(int x, int y) {
  int realY = 15 - y;  // переворачиваем по вертикали
  if (realY % 2 == 0) {
    return realY * 16 + x;
  } else {
    return realY * 16 + (15 - x);
  }
}

void drawDigit16(byte num, int xOffset, int yOffset, uint32_t color, Adafruit_NeoPixel &s) {
  for (int y = 0; y < 16; y++) {
    int src_row = y / 2;
    for (int x = 0; x < 8; x++) {
      bool on = (digits[num][src_row] & (1 << (7 - x)));
      int xi = x + xOffset;
      int yi = y + yOffset; // 👈 добавили вертикальное смещение
      if (xi >= 0 && xi < 16 && yi >= 0 && yi < 16) {
        int idx = xy2index16(xi, yi);
        s.setPixelColor(idx, on ? color : 0);
      }
    }
  }
...
void drawNumberSingle(int value, Adafruit_NeoPixel &s, uint32_t color) {
  ...
  int ones = value % 10;
  s.clear();
  drawDigit16(tens, 0, 1, color, s);  // 👈 добавлен yOffset = 1
  drawDigit16(ones, 8, 1, color, s);  // 👈 добавлен yOffset = 1
  s.show();
}
  ...
  if ((millis() - lastDebounce1) > debounceDelay) {
    ...
    if (reading1 != buttonState1) {
      ...
      if (buttonState1 == LOW && !running2) {
        ...
        lastUpdate2 = millis();  // сброс времени для честного старта
drawNumberSingle(counter2, strip2, normalColor);
  drawNumberSingle(counter1, strip1, pauseColor);
      }
    }
  }
...
lastUpdate1 = millis();  // сброс времени для честного старта
drawNumberSingle(counter1, strip1, normalColor);
  drawNumberSingle(counter2, strip2, pauseColor);
      }
    }
  }
